const log = (...args: any[]) => console.error("[whatsapp]", ...args);
const err = (...args: any[]) => console.error("[whatsapp:error]", ...args);
/**
...
function loadStore() {
    try {
        ...
        for (const [id, chat] of Object.entries(data.chats || {})) {
            chats.set(id, chat as Chat);
            if ((chat as any).name) nameCache.set(id, (chat as any).name);
            // Build LID mapping from chat metadata
            if ((chat as any).lid) {
                jidToLid.set(id, (chat as any).lid);
                lidToJid.set((chat as any).lid, id);
            }
        }
        for (const [id, contact] of Object.entries(data.contacts || {})) {
            ...
            if ((contact as any).lid) {
                jidToLid.set(id, (contact as any).lid);
                lidToJid.set((contact as any).lid, id);
            }
        }
        for (const [jid, msgs] of Object.entries(data.messages || {})) {
            messageStore.set(jid, msgs as WAMessage[]);
            // Extract pushNames from stored messages (WhatsApp display names)
            for (const m of msgs as any[]) {
                if (m.pushName) {
                    const sender = m.key?.participant || m.key?.remoteJid;
                    if (sender) nameCache.set(sender, m.pushName);
                }
            }
        }
        for (const [id, name] of Object.entries(data.names || {})) nameCache.set(id, name as string);
        for (const [jid, lid] of Object.entries(data.jidToLid || {})) {
            jidToLid.set(jid, lid as string);
            lidToJid.set(lid as string, jid);
        }
...
function buildLidMappingFromNames() {
    ...
    const phoneContacts = new Map<string, string>(); // name → phone JID
    for (const [id, contact] of contacts) {
        if (id.endsWith("@s.whatsapp.net")) {
            const name = (contact as any).name || (contact as any).notify;
            if (name) phoneContacts.set(name.toLowerCase(), id);
        }
    }
...
export interface SlackAuth { token: string; cookie?: string; } // unused, just for TS
...
export async function connect(): Promise<WASocket> {
    ...
    sock.ev.on("messaging-history.set", (data) => {
        for (const chat of data.chats) {
            ...
            if ((chat as any).name) nameCache.set(chat.id, (chat as any).name);
            if ((chat as any).lid) {
                jidToLid.set(chat.id, (chat as any).lid);
                lidToJid.set((chat as any).lid, chat.id);
            }
        }
        for (const contact of data.contacts) {
            ...
            const name = (contact as any).name || (contact as any).notify;
            if (name) nameCache.set(contact.id, name);
            if ((contact as any).lid) {
                jidToLid.set(contact.id, (contact as any).lid);
                lidToJid.set((contact as any).lid, contact.id);
            }
        }
        for (const msg of data.messages) {
            ...
            if (msg.pushName) {
                ...
                if (sender) nameCache.set(sender, msg.pushName);
            }
        }
        buildLidMappingFromNames();
        saveStore();
    ...
    sock.ev.on("chats.upsert", (newChats) => {
        for (const chat of newChats) {
            if (chat.id) chats.set(chat.id, { ...chats.get(chat.id), ...chat } as Chat);
        }
        saveStore();
    });
    ...
    sock.ev.on("chats.update", (updates) => {
        for (const update of updates) {
            ...
            if (existing) chats.set(update.id, { ...existing, ...update } as Chat);
        }
    });
    ...
    sock.ev.on("contacts.update", (updates) => {
        for (const update of updates) {
            ...
            if (existing) contacts.set(update.id, { ...existing, ...update } as Contact);
        }
    });
 ...
 */
async function waitForSync(timeoutMs = 10000): Promise<void> {
...
export async function getChats(limit = 20): Promise<Array<{
    id: string;
    name: string;
    lastMessage: string;
    ...
    return chatList.map((chat) => {
        ...
        return {
            ...
            lastTimestamp: (chat as any).conversationTimestamp || 0,
...
export async function replyToMessage(chatId: string, text: string, quotedMessageId: string): Promise<{ id: string }> {
    const s = ensureConnected();
    const jid = normalizeJid(chatId);
    const result = await s.sendMessage(jid, { text }, {
        quoted: { key: { remoteJid: jid, id: quotedMessageId } } as any,
    });
    return { id: result?.key?.id || "" };
}
