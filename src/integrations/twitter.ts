...
let cachedConfig: TwitterConfig | null = null;
 ...
 */
async function getTwitterConfig(auth: TwitterAuth): Promise<TwitterConfig> {
...
async function twitterApi<T = any>(
    auth: TwitterAuth,
    path: string,
...
export async function getProfile(auth: TwitterAuth, screenName: string): Promise<any> {
    const config = await getTwitterConfig(auth);
    const qid = config.queryIds.UserByScreenName;
    if (!qid) throw new Error("UserByScreenName query ID not available");
...
/** Get a user's tweets */
export async function getUserTweets(auth: TwitterAuth, screenName: string, count = 20): Promise<any[]> {
    const config = await getTwitterConfig(auth);

    // Get user ID first
    const qidUser = config.queryIds.UserByScreenName;
    if (!qidUser) throw new Error("UserByScreenName query ID not available");
    ...
    if (!qid) throw new Error("UserTweets query ID not available");
...
/** Get home timeline */
export async function getTimeline(auth: TwitterAuth, count = 20): Promise<any[]> {
    const config = await getTwitterConfig(auth);
    const qid = config.queryIds.HomeTimeline;
    if (!qid) throw new Error("HomeTimeline query ID not available");
...
/** Post a tweet */
export async function createTweet(auth: TwitterAuth, text: string, replyToId?: string): Promise<any> {
    const config = await getTwitterConfig(auth);
    const qid = config.queryIds.CreateTweet;
    if (!qid) throw new Error("CreateTweet query ID not available");
...
/** Search tweets */
export async function searchTweets(auth: TwitterAuth, query: string, count = 20, product: "Latest" | "Top" = "Latest"): Promise<any[]> {
    const config = await getTwitterConfig(auth);
    const qid = config.queryIds.SearchTimeline;
    if (!qid) throw new Error("SearchTimeline query ID not available");
